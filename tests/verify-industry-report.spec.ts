...
// ─── Test ─────────────────────────────────────────────────────────────────────
test('Industry Report – full flow', async ({ page }) => {
  ...
  // ── STEP 4: Click "Industry Report" sub-menu item ───────────────────────────
  console.log('\n══ STEP 4: Click Industry Report ══');
  await findAndClick(page, [
    () => page.locator("xpath=//ul[contains(@class,'el-menu--popup')]//li[contains(@class,'el-menu-item') and normalize-space()='Industry Report']"),
    () => page.locator('.el-menu--popup .el-menu-item').filter({ hasText: /^Industry Report$/ }).first(),
    () => page.locator('li.el-menu-item').filter({ hasText: /Industry Report/i }).first(),
    () => page.getByRole('menuitem', { name: /Industry Report/i }),
  ], 'Industry Report menu item');

  await page.waitForLoadState('networkidle');
  await page.waitForTimeout(1500);
  console.log('✓ Industry Report page loaded');
  ...
  // Click "Content Type" from the visible dropdown
  await findAndClick(page, [
  ...
  // Also check "No results found" is NOT present
  ...
  console.log('\n🎉 Industry Report Test COMPLETED');
});
